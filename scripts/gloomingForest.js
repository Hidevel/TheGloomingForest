...
render = function() {
	...
	keysInterpreter =function() {
		...
		if (keyboard[38]){ // up arrow key	
			...
			if (Math.abs(spirit.position.x) >= Math.abs(mapSize/2 + 0.5) || Math.abs(spirit.position.z) >= Math.abs(mapSize/2 + 0.5) ){		
				restart();
			}
...
handleCollision=function(collided_with){
	...
	if(collided_with.name.includes("mushroom")){
		for(var i=0;i<numMushrooms;i++){
			transparency += 0.02;
			if (transparency > 1.0) transparency = 1;
